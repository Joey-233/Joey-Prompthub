...
function getBreakpoint(desktopMinWidth: number): Breakpoint {
  if (typeof window.matchMedia !== 'function') return 'desktop'
  if (window.matchMedia(`(min-width: ${desktopMinWidth}px)`).matches) return 'desktop'
  if (window.matchMedia('(min-width: 1025px)').matches) return 'tablet'
  return 'mobile'
}

function useBreakpoint(desktopMinWidth: number) {
  const [breakpoint, setBreakpoint] = useState<Breakpoint>(() => getBreakpoint(desktopMinWidth))
  useEffect(() => {
...
export function WorkspaceLayout({
  ...
  detailLabel = '详情',
  className = '',
  desktopMinWidth = 1320
}: WorkspaceLayoutProps) {
  const breakpoint = useBreakpoint(desktopMinWidth)
  ...
  useEffect(() => {
    ...
    if (!drawer) return
    pendingFocus.current = (drawer === 'resource' ? resourceTrigger : detailTrigger).current
    // A breakpoint transition changes the interaction model, so any open drawer must close.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setDrawer(null)
    // This effect intentionally runs only when the responsive mode changes.
    ...
    function cleanup() {
      ...
      try {
        if (
          ...
          (!source.hasPointerCapture || source.hasPointerCapture(pointerId))
        )
  ...
  const separator = (pane: Pane) => {
    ...
    return (
      <div
        ...
        aria-label={`调整${resourcePane ? resourceLabel : detailLabel}面板宽度`}
        aria-valuemin={resourcePane ? 180 : 280}
        aria-valuemax={resourcePane ? 320 : 480}
        aria-valuenow={value}
        onKeyDown={separatorKey(pane)}
        onPointerDown={startResize(pane)}
        onDoubleClick={() => setPaneWidth(pane, resourcePane ? 220 : 320)}
      />
    )
  }
  ...
  return (
    <div
      ...
      style={
        {
          '--resource-width': `${layout.resourceWidth}px`,
          '--detail-width': `${layout.detailWidth}px`
        } as CSSProperties
      }
    >
      <div className="workspace-tools">
        {resource &&
          (breakpoint === 'mobile' ? (
            <button ref={resourceTrigger} type="button" onClick={() => setDrawer('resource')}>
              打开资源面板
            </button>
          ) : (
            <button
              type="button"
              onClick={() => setPaneCollapsed('resource', !layout.resourceCollapsed)}
            >
              {layout.resourceCollapsed ? '展开资源面板' : '收起资源面板'}
            </button>
          ))}
        {detail &&
          (breakpoint === 'desktop' ? (
            <button
              type="button"
              onClick={() => setPaneCollapsed('detail', !layout.detailCollapsed)}
            >
              {layout.detailCollapsed ? '展开详情面板' : '收起详情面板'}
            </button>
          ) : (
            <button ref={detailTrigger} type="button" onClick={() => setDrawer('detail')}>
              打开详情面板
            </button>
          ))}
      </div>
      <div className="workspace-panes">
        {inlineResource && (
          <>
            <aside
              ...
              style={{ minWidth: 180, flexShrink: 1 }}
            >
              {resource}
            </aside>
            {separator('resource')}
        ...
        {inlineDetail && (
          <>
            {separator('detail')}
            <aside
              className="workspace-pane detail-pane"
              role="region"
              aria-label={detailLabel}
              style={{ minWidth: 280, flexShrink: 1 }}
            >
              {detail}
            </aside>
          </>
