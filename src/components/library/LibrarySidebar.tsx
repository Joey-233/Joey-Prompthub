...
export function LibrarySidebar() {
  ...
  const tagButton = (tag: string | null, label = tag ?? '全部提示词') => (
    <button aria-label={label} className="library-nav-item" data-active={filterTag === tag} role="tab" aria-selected={filterTag === tag} type="button" onClick={() => setFilterTag(tag)}>
      <span>{label}</span>{tag && <span className="library-nav-count">{tags.find(([value]) => value === tag)?.[1] ?? 0}</span>}
    </button>
  )

  return <nav className="library-sidebar" aria-label="提示词导航">
