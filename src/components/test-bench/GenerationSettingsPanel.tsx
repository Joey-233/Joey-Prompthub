...
export function clampNumeric(value: string, min: number, max?: number) {
...
export function GenerationSettingsPanel({ content, loading, providerId, params, onProviderChange, onParamsChange, onGenerate }: {
  ...
  const numberField = (label: string, key: 'width' | 'height' | 'steps' | 'count', min = 1, max?: number) => <label className="bench-param-field"><span>{label}</span><input aria-label={label} type="number" min={min} max={max} value={params[key] ?? ''} onChange={(e) => onParamsChange({ [key]: clampNumeric(e.target.value, min, max) })} /></label>
