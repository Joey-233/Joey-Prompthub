...
export function GenerationSettingsPanel({ content, loading, providerId, params, onProviderChange, onParamsChange, onGenerate }: {
  ...
  return <div className="generation-settings">
    ...
    {caps.steps ? <label className="bench-param-field"><span>步数</span><input aria-label="步数" type="number" min={caps.steps.min} max={caps.steps.max} value={params.steps ?? caps.steps.default} onChange={(e) => onParamsChange({ steps: Math.max(caps.steps!.min, Math.min(caps.steps!.max, Number(e.target.value) || caps.steps!.min)) })} /></label> : null}
