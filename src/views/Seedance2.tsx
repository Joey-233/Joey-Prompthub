...
export function Seedance2() {
  ...
  const load = (rec: Seedance2TemplateRecord) => { setCurrentId(rec.id); setTitle(rec.title); setDraft(rec.data); setDirty(false) }
  const createNew = () => { setCurrentId(null); setTitle('未命名模板'); setDraft(emptyTemplate()); setDirty(true) }
  ...
  const toggleSection = (section: Section) => {
    const panel = document.getElementById(`seedance-section-${section}`)
    if (activeSection === section && !panel?.contains(document.activeElement)) setActiveSection(null)
  ...
  const accordion = (section: Section, children: ReactNode) => <section ref={(node) => { refs.current[section] = node }} className="s2-accordion">
    <h2><button type="button" aria-expanded={activeSection === section} aria-controls={`seedance-section-${section}`} onMouseDown={(event) => { if (document.getElementById(`seedance-section-${section}`)?.contains(document.activeElement)) event.preventDefault() }} onClick={() => toggleSection(section)}>{sectionNames[section]}</button></h2>
  ...
  const main = <main className="s2-main" aria-label="Seedance2 编辑器">
    <div className="s2-toolbar"><label><span className="sr-only">模板标题</span><input className="s2-input s2-title-input" value={title} onChange={(e) => { setTitle(e.target.value); setDirty(true) }} /></label><button className="s2-btn s2-btn-primary" disabled={!dirty && !!currentId} onClick={() => void save()}>{currentId ? '保存' : '保存为新模板'}</button>{currentId && <button className="s2-btn" onClick={() => dirty ? request({ type: 'delete' }) : setConfirmDelete(true)}>删除</button>}</div>
  ...
  return <><WorkspaceLayout resource={resource} resourceLabel="Seedance2 资源" main={main} detail={detail} detailLabel="实时预览" />{pending && <UnsavedChangesDialog saving={saving} error={saveError} onSave={() => void saveAndContinue()} onDiscard={() => perform(pending)} onCancel={() => { setPending(null); setSaveError(null) }} />}{confirmDelete && <div className="s2-dialog-backdrop"><section className="s2-dialog" role="dialog" aria-modal="true" aria-labelledby="s2-delete-title"><h2 id="s2-delete-title">删除模板</h2><p>确定删除“{title}”吗？</p><div className="s2-dialog-actions"><button className="s2-btn" onClick={() => setConfirmDelete(false)}>取消</button><button className="s2-btn s2-btn-primary" onClick={() => { setConfirmDelete(false); perform({ type: 'delete' }) }}>确认删除</button></div></section></div>}</>
}
