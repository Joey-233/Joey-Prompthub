...
describe("Settings", () => {
  it("organizes settings into accessible category, content, and help regions", async () => {
    ...
    expect(
      screen.getByRole("region", { name: "设置分类" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: "设置内容" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: "配置状态与帮助" }),
    ).toBeInTheDocument();
    const content = screen.getByRole("region", { name: "设置内容" });
    expect(
      within(content).getByRole("heading", { name: "AI 服务" }),
    ).toBeInTheDocument();
    expect(
      within(content).queryByRole("heading", { name: "视觉模型" }),
    ).not.toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: "视觉模型" }));
    expect(
      within(content).getByRole("heading", { name: "视觉模型" }),
    ).toBeInTheDocument();
    expect(
      within(content).queryByRole("heading", { name: "AI 服务" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows a failed write beside the section and retries the exact value", async () => {
    ...
    rejectWrites = false;
    await user.click(
      within(screen.getByRole("region", { name: "设置内容" })).getByRole(
        "button",
        { name: "保存失败，点击重试" },
      ),
    );
    ...
    expect(
      await within(screen.getByRole("region", { name: "设置内容" })).findByText(
        "已保存",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("keeps the latest write status when an older write finishes last", async () => {
    ...
    expect(within(screen.getByRole("region", { name: "设置内容" })).getByText("保存中…")).toBeInTheDocument();
    ...
    expect(
      within(screen.getByRole("region", { name: "设置内容" })).getByText(
        "已保存",
      ),
    ).toBeInTheDocument();
  });

  it("serializes same-key writes at the side-effect layer and continues after failure", async () => {
      ...
      .mockImplementation((_key, value) => {
        if (value === "first")
          return new Promise<void>((_resolve, reject) => {
            finishFirst = () => reject(new Error("first failed"));
          });
    ...
    await waitFor(() => expect(window.promptHub.settings.set).toHaveBeenCalledTimes(1));
  ...
  it("does not overwrite a local edit when the initial settings load finishes later", async () => {
    ...
    window.promptHub.settings.list = vi.fn().mockReturnValue(
      new Promise((resolve) => {
        finishLoad = resolve;
      }),
    );
    ...
    await act(async () =>
      finishLoad({ ai_model: "remote-model", theme_mode: "dark" }),
    );
  ...
  it("retries both persistence targets when launch-at-login OS update fails", async () => {
    ...
    const retry = await within(
      screen.getByRole("region", { name: "设置内容" }),
    ).findByRole("button", { name: "保存失败，点击重试" });
    await user.click(retry);
    expect(window.promptHub.settings.set).toHaveBeenCalledTimes(2);
    expect(window.promptHub.settings.set).toHaveBeenLastCalledWith(
      "launch_at_login",
      true,
    );
    expect(window.promptHub.system.setLaunchAtLogin).toHaveBeenCalledTimes(2);
    expect(window.promptHub.system.setLaunchAtLogin).toHaveBeenLastCalledWith(
      true,
    );
  });
  ...
  it("reports launch-at-login failure when the settings write rejects", async () => {
    ...
    await user.click(screen.getByLabelText("开机自启"));
    expect(
      await within(screen.getByRole("region", { name: "设置内容" })).findByRole(
        "button",
        { name: "保存失败，点击重试" },
      ),
    ).toBeInTheDocument();
  ...
  it("serializes rapid launch-at-login updates across both persistence targets", async () => {
    const user = userEvent.setup();
    let finishTrue!: () => void;
  ...
  it("accepts a vision override when the followed AI model is empty", async () => {
    const user = userEvent.setup();
    window.promptHub.settings.list = vi
      .fn()
      .mockResolvedValue({
        ai_model: "",
        vision_model: "gpt-4o",
  ...
  it("keeps advanced fields collapsed until expanded", async () => {
    ...
    expect(screen.getByLabelText("默认模型")).not.toBeVisible();
  ...
  it("reports the exact missing AI dependencies in vision follow mode", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "视觉模型" }));
    await waitFor(() =>
      expect(window.promptHub.settings.list).toHaveBeenCalled(),
    );
    await user.click(screen.getByRole("button", { name: "检查配置" }));
    expect(
      screen.getByText("缺少：AI Base URL、AI API Key"),
    ).toBeInTheDocument();
  });

  it("updates followed vision completeness when parent secure checks resolve after navigation", async () => {
    const user = userEvent.setup();
    const resolvers = new Map<string, (value: boolean) => void>();
    window.promptHub.secure.has = vi.fn().mockImplementation(
      (key: string) =>
    ...
    await user.click(screen.getByRole("button", { name: "视觉模型" }));
    await act(async () => {
      resolvers.get("ai.apiKey")?.(true);
      resolvers.get("vision.apiKey")?.(false);
    });
    await user.click(screen.getByRole("button", { name: "检查配置" }));
    expect(screen.getByText("配置完整")).toBeInTheDocument();
    expect(window.promptHub.secure.has).toHaveBeenCalledTimes(2);
  });
  ...
  it("reports a secure reveal failure without an unhandled rejection", async () => {
    const user = userEvent.setup();
    window.promptHub.secure.has = vi.fn().mockResolvedValue(true);
    window.promptHub.secure.reveal = vi
      .fn()
  ...
  it("switching AI preset auto-fills baseURL and default model", async () => {
    ...
    const aiPresetSelect = screen.getAllByLabelText(
      "服务商预设",
  ...
  it("vision source defaults to follow mode without a dedicated key field", async () => {
    ...
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "视觉模型" }));

    expect(
      (screen.getByLabelText("识图服务来源") as HTMLSelectElement).value,
    ).toBe("follow");
    expect(
      screen.queryByLabelText("识图 API Key（可留空）"),
    ).not.toBeInTheDocument();
  });
