...
import { AI_PRESETS, IMAGE_PRESETS, findAiPreset, findImagePreset } from '../services/ai/presets'

type SettingsMap = Record<string, unknown>
type SaveState = { state: 'idle' | 'saving' | 'saved' | 'error'; value?: unknown }

const defaults: SettingsMap = { ai_preset: 'openai', ai_base_url: 'https://api.openai.com/v1', ai_model: 'gpt-4.1-mini', vision_preset: 'follow', vision_base_url: '', vision_model: '', image_preset: 'openai-image', image_base_url: 'https://api.openai.com/v1', image_model: 'gpt-image-1', theme_mode: 'system', launch_at_login: false }
const sectionKeys: Record<SettingsCategory, string[]> = { ai: ['ai_preset', 'ai_base_url', 'ai_model'], vision: ['vision_preset', 'vision_base_url', 'vision_model'], image: ['image_preset', 'image_base_url', 'image_model'], data: ['theme_mode', 'launch_at_login'] }
const descriptions: Record<SettingsCategory, string> = { ai: '配置文本生成与提示词优化使用的服务。', vision: '配置快速录入时的图片理解模型，可跟随 AI 服务。', image: '配置测试台使用的图像生成服务。', data: '管理本地数据、主题与桌面应用行为。' }
...
export function Settings() {
  ...
  useEffect(() => {
    let mounted = true
    void window.promptHub.settings.list().then((loaded) => {
      if (!mounted) return
      setSettings((current) => {
        const safe = Object.fromEntries(Object.entries(loaded).filter(([key]) => !edited.current.has(key)))
        return { ...current, ...safe }
      })
    }).catch(() => undefined)
  ...
  function handleVisionPreset(id: string) {
    ...
    if (preset && !preset.baseUrlEditable) values.vision_base_url = preset.baseURL
    if (preset?.suggestedVisionModels?.[0]) values.vision_model = preset.suggestedVisionModels[0]
    setMany(values)
  }
  ...
  function handleImagePreset(id: string) {
    ...
    if (preset.baseURL && !preset.baseUrlEditable) values.image_base_url = preset.baseURL
    if (preset.defaultModel) values.image_model = preset.defaultModel
    setMany(values)
  }

  const aiPreset = useMemo(() => findAiPreset(String(settings.ai_preset)), [settings.ai_preset])
  const visionId = String(settings.vision_preset)
  const visionPreset = visionId === 'follow' ? null : findAiPreset(visionId)
  const imagePreset = useMemo(() => findImagePreset(String(settings.image_preset)), [settings.image_preset])
  const activeStates = sectionKeys[active].map((key) => statuses[key]).filter(Boolean)
  const sectionState = activeStates.some((item) => item.state === 'error') ? 'error' : activeStates.some((item) => item.state === 'saving') ? 'saving' : activeStates.some((item) => item.state === 'saved') ? 'saved' : 'idle'
  const failed = sectionKeys[active].map((key) => [key, statuses[key]] as const).find(([, value]) => value?.state === 'error')
