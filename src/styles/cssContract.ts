...
export function findLegacyRadii(css: string, allowedSelectors: ReadonlySet<string> = new Set()): LegacyRadius[] {
  ...
  for (const block of source.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    ...
    for (const declaration of block[2].split(';')) {
      ...
      if (separator < 0 || declaration.slice(0, separator).trim().toLowerCase() !== 'border-radius') continue
      const value = declaration.slice(separator + 1)
      const radii = Array.from(value.matchAll(/(?:^|\s)(\d+(?:\.\d+)?)px(?=\s|\/|!|$)/gi), (match) => Number(match[1]))
